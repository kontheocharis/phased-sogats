...
from-builtin : {A : Set ℓ} {x y : A} → x Agda.Builtin.Equality.≡ y → x ≡ y
from-builtin Agda.Builtin.Equality.refl = refl
...
-- Here we have various versions of Σ that live in Prop
...
data ∃ (A : Set ℓ) (B : A → Prop ℓ') : Prop (ℓ ⊔ ℓ') where
  _,∃_ : (a : A) → B a → ∃ A B

syntax ∃ A (λ x → B) = ∃[ x ∈ A ] B

∃-elim : ∀ {A : Set ℓ} {B : A → Prop ℓ'} {C : ∃ A B → Prop ℓ}
  → ((a : A) → (b : B a) → C (a ,∃ b)) → (p : ∃ A B) → C p
∃-elim f (a ,∃ x) = f a x

record _∧_ (A : Prop ℓ) (B : Prop ℓ') : Prop (ℓ ⊔ ℓ') where
  constructor _,∧_
  field
    fst : A
    snd : B
...
postulate
  coe-Σ : (Σ A B ≡ Σ A' B') → (∥Σ∥ (A ≡ A') (λ p → ∀ x → B x ≡ B' (coe p x)))
  coe-pair :
    {a : A} {b : B a}
    {p : Σ A B ≡ Σ A' B'}
...
opaque
  ...
  ≡Σ : {p₁ p₂ : Σ A B} (p : p₁ ≡ p₂) → ∥Σ∥ (p₁ .proj₁ ≡ p₂ .proj₁) (λ p → subst B p (p₁ .proj₂) ≡ (p₂ .proj₂))
  ≡Σ refl = refl ,P refl
